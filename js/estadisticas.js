...
function crearEstadisticas(matches) {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    // for (let g = 0; g < estadisticas.length; g++) {
    //   estadisticas[g].top5 = matches[g].score.fullTime.
...
crearEstadisticas(matches.matches);
...
function nuevatabla(estadisticas) {
  ...
  for (var h = 0; h < 5; h++) {
    ...
    media.textContent = estadisticas[h].avg
...
function crearEstadisticas2(matches) {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    for (var b = 0; b < estadisticas2.length; b++) {

      if (id_homeTeam == estadisticas2[b].id) {
        foundHomeTeam = estadisticas2[b];

      }
      if (id_awayTeam == estadisticas2[b].id) {
        foundAwayTeam = estadisticas2[b];
      }
    }
    ...
    if (foundAwayTeam == undefined) {
      ...
      estadisticas2.push(objeto2);
    } else {
      foundAwayTeam.goals += goals_awayTeam;
      foundAwayTeam.matches++;
    }
    for (let c = 0; c < estadisticas2.length; c++) {
      estadisticas2[c].avg = estadisticas2[c].goals / estadisticas2[c].matches;
    }
    // for (let g = 0; g < estadisticas.length; g++) {
    //   estadisticas[g].top5 = matches[g].score.fullTime.
